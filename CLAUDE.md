...
- Vanilla JavaScript (ES5-compatible IIFEs for frontend), Python 3.10+ (processing pipeline) + Leaflet.js (vendored), Firebase SDK v11 (vendored), ffmpeg/ffprobe (CLI) (013-native-video-playback)
- Firebase Storage (Spark free tier: 5 GB storage, 1 GB/day downloads), local JSON manifests (013-native-video-playback)
...
## Recent Changes
- 013-native-video-playback: Added Vanilla JavaScript (ES5-compatible IIFEs for frontend), Python 3.10+ (processing pipeline) + Leaflet.js (vendored), Firebase SDK v11 (vendored), ffmpeg/ffprobe (CLI)
